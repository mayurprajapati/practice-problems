...
class Utils {
    public static boolean isLeafNode(TreeNode node) {
        return Objects.requireNonNull(getChildrenFromTreeNode(node)).size() == 0;
    }

    public static ArrayList<Tree> getChildrenFromTreeNode(TreeNode node) {
        try {
            java.lang.reflect.Field field = node.getClass().getDeclaredFields()[0];
            field.setAccessible(true);
            return (ArrayList<Tree>) field.get(node);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
